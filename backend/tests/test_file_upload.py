...
class TestFileUploadSuccess:
    ...
    def test_upload_pdf_as_owner(self, client, gp_headers, created_chat, tmp_path):
        with patch("src.services.chat_service.UPLOAD_DIR", tmp_path):
            resp = client.post(
                f"/chats/{created_chat['id']}/files",
                files={"file": ("guideline.pdf", io.BytesIO(_PDF_BYTES), "application/pdf")},
                headers=gp_headers,
            )
        assert resp.status_code == 201
        data = resp.json()
    ...
    def test_upload_multiple_files_to_same_chat(self, client, gp_headers, created_chat, tmp_path):
        with patch("src.services.chat_service.UPLOAD_DIR", tmp_path):
            for i in range(3):
                resp = client.post(
                    f"/chats/{created_chat['id']}/files",
    ...
    def test_file_size_stored_correctly(self, client, gp_headers, created_chat, tmp_path):
    ...
    def test_uploaded_file_appears_in_chat_detail(self, client, gp_headers, created_chat, tmp_path):
        with patch("src.services.chat_service.UPLOAD_DIR", tmp_path):
            client.post(
                f"/chats/{created_chat['id']}/files",
    ...
    def test_multiple_files_all_appear_in_chat_detail(self, client, gp_headers, created_chat, tmp_path):
        with patch("src.services.chat_service.UPLOAD_DIR", tmp_path):
...
class TestFileUploadAuth:
    ...
    def test_specialist_can_upload_to_assigned_chat(
        self, client, gp_headers, specialist_headers, registered_specialist, created_chat, tmp_path
    ):
        ...
        with patch("src.services.chat_service.UPLOAD_DIR", tmp_path):
            resp = client.post(
                f"/chats/{created_chat['id']}/files",
                files={"file": ("specialist_note.txt", io.BytesIO(b"Specialist note."), "text/plain")},
                headers=specialist_headers,
            )
        assert resp.status_code == 201
