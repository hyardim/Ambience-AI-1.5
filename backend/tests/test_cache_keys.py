...
def test_specialist_queue_key_formats():
    assert cache_keys.specialist_queue("neurology") == "cache:specialist:queue:neurology"
...
def test_admin_audit_log_and_notification_key_formats():
    ...
    assert cache_keys.notifications(7, unread_only=True) == "cache:user:7:notifications:unread"
    assert cache_keys.notifications_pattern(7) == "cache:user:7:notifications:*"
    assert cache_keys.notifications_unread_count(7) == "cache:user:7:notifications:count:unread"
