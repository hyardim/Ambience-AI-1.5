...
class TestUpdateChat:
    ...
    def test_update_other_users_chat_fails(
        self, client, second_gp_headers, created_chat
    ):
        resp = client.patch(
            f"/chats/{created_chat['id']}",
            json={"title": "Hijack"},
...
class TestSubmitForReview:
    ...
    def test_submit_other_users_chat_fails(
        self, client, second_gp_headers, created_chat
    ):
        resp = client.post(
            f"/chats/{created_chat['id']}/submit", headers=second_gp_headers
        )
        assert resp.status_code == 404
