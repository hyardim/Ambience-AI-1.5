...
class TestUpdateProfile:
    def test_update_full_name_success(self, client, gp_headers):
        resp = client.patch(
            "/auth/profile", json={"full_name": "New Name"}, headers=gp_headers
        )
    ...
    def test_update_specialty_success(self, client, gp_headers):
        resp = client.patch(
            "/auth/profile", json={"specialty": "cardiology"}, headers=gp_headers
        )
        assert resp.status_code == 200
    ...
    def test_update_password_without_current_fails(self, client, gp_headers):
        resp = client.patch(
            "/auth/profile", json={"new_password": "NewPass456!"}, headers=gp_headers
        )
        assert resp.status_code == 400
    ...
    def test_update_reflects_in_me(self, client, gp_headers):
        client.patch(
            "/auth/profile", json={"full_name": "Updated Name"}, headers=gp_headers
        )
