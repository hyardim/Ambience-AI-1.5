...
class TestArchiveChat:
    ...
    def test_archived_chat_still_fetchable_by_id(
...
class TestSendMessage:
    ...
    def test_send_message_returns_mock_ai_response(
        self, client, gp_headers, created_chat
    ):
        chat_id = created_chat["id"]
...
class TestListChatsFiltering:
    ...
    def test_filter_by_date_range(self, client, gp_headers):
        ...
        resp = client.get(
            "/chats/?date_from=2000-01-01T00:00:00&date_to=2099-12-31T23:59:59",
            headers=gp_headers,
        )
        assert resp.status_code == 200
        assert len(resp.json()) >= 1
    ...
    def test_combined_search_and_specialty(self, client, gp_headers):
        ...
        resp = client.get(
            "/chats/?search=headache&specialty=neurology", headers=gp_headers
        )
        assert resp.status_code == 200
        data = resp.json()
        assert len(data) == 1
