...
from src.services import admin_service, chat_service, specialist_service
...
def test_list_chats_cache_hit_uses_cached_data(monkeypatch):
    ...
    monkeypatch.setattr(
        chat_service.chat_repository, "list_for_user", fake_list_for_user
    )
...
def test_list_chats_cache_miss_sets_cache(monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        "list_for_user",
        lambda *_args, **_kwargs: [fake_chat],
    )
...
def test_create_chat_invalidates_list_cache(monkeypatch):
    ...
    monkeypatch.setattr(
        chat_service.chat_repository, "create", lambda *_args, **_kwargs: fake_chat
    )
    monkeypatch.setattr(
        chat_service.audit_repository, "log", lambda *_args, **_kwargs: None
    )
    monkeypatch.setattr(
        chat_service.cache, "delete_pattern_sync", fake_delete_pattern_sync
    )
...
def test_list_notifications_cache_hit_uses_cached_data(monkeypatch):
    ...
    monkeypatch.setattr(
        notification_service.cache, "get_sync", lambda *_a, **_k: cached
    )
    monkeypatch.setattr(
        notification_service.notification_repository,
        "list_for_user",
        lambda *_a, **_k: (_ for _ in ()).throw(
            AssertionError("repo should not be called when cache hits")
        ),
    )
