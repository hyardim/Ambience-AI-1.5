...
class TestSSEEvent:
    def test_encode_produces_valid_sse_frame(self):
        ev = SSEEvent(
            event="content", data={"chat_id": 1, "message_id": 5, "content": "hello"}
        )
        encoded = ev.encode()
...
class TestReplayBuffer:
    ...
    async def test_latest_content_buffered(self, bus):
        ...
        await bus.publish(
            1, SSEEvent(event="stream_start", data={"chat_id": 1, "message_id": 5})
        )
    ...
    async def test_terminal_event_replayed_with_sentinel(self, bus):
        ...
        await bus.publish(
            1, SSEEvent(event="complete", data={"content": "final", "citations": []})
        )
