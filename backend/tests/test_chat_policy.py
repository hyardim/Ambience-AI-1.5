...
def _make_chat(
    ...
    return SimpleNamespace(
        ...
        specialty=specialty, specialist_id=specialist_id,
    )
...
class TestCanViewChat:
    ...
    def test_specialist_matching_specialty_can_view_submitted(self):
        chat = _make_chat(
            user_id=1, status=ChatStatus.SUBMITTED, specialty="neurology",
        )
    ...
    def test_assigned_specialist_can_view(self):
        chat = _make_chat(
            user_id=1, status=ChatStatus.ASSIGNED, specialist_id=SPEC_NEURO.id,
        )
        assert can_view_chat(SPEC_NEURO, chat) is True
