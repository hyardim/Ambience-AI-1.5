...
def _enable_verification_mode(monkeypatch) -> None:
    monkeypatch.setattr('src.core.config.settings.NEW_USERS_REQUIRE_EMAIL_VERIFICATION', True)
    monkeypatch.setattr('src.core.config.settings.ALLOW_LEGACY_UNVERIFIED_LOGIN', False)


def test_register_creates_unverified_user_and_token(client, db_session, gp_user_payload, monkeypatch):
...
def test_confirm_email_verification_with_valid_token(client, db_session, gp_user_payload, monkeypatch):
    _enable_verification_mode(monkeypatch)
    captured = _capture_verification_link(monkeypatch)
...
def test_resend_is_generic_and_rate_limited(client, gp_user_payload, monkeypatch):
    ...
    responses = [
        client.post('/auth/resend-verification', json={'email': gp_user_payload['email']})
        for _ in range(7)
    ]
...
def test_audit_entries_created_for_verification_flow(client, db_session, gp_user_payload, monkeypatch):
    _enable_verification_mode(monkeypatch)
    captured = _capture_verification_link(monkeypatch)
...
def test_legacy_users_default_to_verified_on_register_when_flag_off(client, gp_user_payload, monkeypatch):
