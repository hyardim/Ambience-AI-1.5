...
_INSTRUCTIONS_NEW = (
    ...
    "When evidence is available, prefer 6-10 short sentences rather than a "
    ...
    "Do not state that a referral criterion is absent unless that absence is "
    "explicitly stated in the question or the cited passage. "
    "If the question asks about referral urgency or pathway, include one direct "
    "referral sentence with explicit urgency wording from the cited passage. "
    "Prefer concrete criteria wording from citations instead of vague phrasing "
    "such as 'if criteria apply'. "
    "Avoid hedge-only phrasing such as 'might be considered' when the cited "
    "guideline gives a direct referral instruction. "
    ...
    "Do not add extra branching criteria that are not asked in the question "
    "(for example additional 'if there is evidence of ...' conditions). "
    "Keep population focus aligned to the question (for example do not add "
    "child-specific advice for an adult-only question unless the passage "
    "explicitly requires it for the asked action). "
