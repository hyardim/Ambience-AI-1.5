...
_INSTRUCTIONS_NEW = (
    ...
    "explicitly. "
    "If the clinician asks whether to start or initiate a treatment in primary "
    ...
    "only when that rationale is explicitly supported by citations. "
    ...
    "Every grounded clinical claim sentence must include a valid indexed "
    "citation [N] or explicitly cite 'Uploaded document'. "
...
def _mode_instructions(answer_mode: str) -> str:
    ...
    if answer_mode == "comparison":
        return (
            ...
            "- Under 'Key differences:' use short bullets beginning exactly "
            "'- Migraine aura:' and '- TIA:' when those conditions are the "
            "comparison pair.\n"
