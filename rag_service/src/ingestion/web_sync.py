        ...
        async with self._lock:
            ...
            async with httpx.AsyncClient() as client:
                for source in allowed_sources:
                    try:
                        discovered = await self._discovery_client.discover_source(
                            source
                        )
                    ...
                    for document in discovered:
                        ...
                        parsed_length = (
                            ...
                            if resolved_content_length
                            and str(resolved_content_length).isdigit()
            ...
            logger.info(
                (
                    "sync.complete discovered=%s new=%s updated=%s unchanged=%s "
                    "ingest_ok=%s ingest_failed=%s errors=%s"
                ),
            ...
            return cast(dict[str, Any], state["last_run"])
