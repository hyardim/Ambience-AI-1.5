...
def _extract_page_title(html: str) -> str | None:
    ...
    if heading is not None:
        text = cast(str, heading.get_text(" ", strip=True))
        if text:
            return text
    title = soup.find("title")
    if title is not None:
        text = cast(str, title.get_text(" ", strip=True))
        if text:
            return text
    return None
    ...
    async def discover_source(
        ...
        async with httpx.AsyncClient() as client:
            ...
            for candidate in candidate_links:
                ...
                title = _extract_page_title(detail.text) or PathLikeTitle.from_url(
                    candidate
                )
    ...
    async def _discover_nice_candidates(
        ...
        *,
        ...
        while queue and len(visited) < MAX_NICE_CRAWL_PAGES:
            ...
            for guideline_url in _extract_nice_guideline_links(
                html, base_url=normalized_page
            ):
            ...
            for child_url in _extract_nice_subcategory_links(
                html, listing_url=start_url
            ):
        ...
        logger.info(
            (
                "sync.discovery.nice source=%s visited=%s "
                "guideline_links=%s direct_pdfs=%s"
            ),
            source.source_name,
            len(visited),
            len(guideline_links),
