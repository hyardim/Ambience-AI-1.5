...
class TestCleanDocument:
    ...
    def test_unicode_normalized(self) -> None:
        raw_doc = make_raw_doc(pages=[make_page(1, blocks=[make_block("ﬁle and ﬂow")])])
        result = clean_document(raw_doc)
        assert result["pages"][0]["blocks"][0]["text"] == "file and flow"

    def test_whitespace_normalized(self) -> None:
        raw_doc = make_raw_doc(
            pages=[make_page(1, blocks=[make_block("hello   world")])]
        )
        result = clean_document(raw_doc)
    ...
    def test_hyphenated_line_break_merged(self) -> None:
        raw_doc = make_raw_doc(
            pages=[make_page(1, blocks=[make_block("treat-\nment")])]
        )
        result = clean_document(raw_doc)
    ...
    def test_bullets_normalized(self) -> None:
        raw_doc = make_raw_doc(
            pages=[make_page(1, blocks=[make_block("• item one\n• item two")])]
        )
        result = clean_document(raw_doc)
    ...
    def test_bbox_preserved(self) -> None:
        ...
        raw_doc = make_raw_doc(
            pages=[make_page(1, blocks=[make_block("Content", bbox=bbox)])]
        )
        result = clean_document(raw_doc)
    ...
    def test_deterministic(self) -> None:
        raw_doc = make_raw_doc(
            pages=[make_page(1, blocks=[make_block("ﬁle treat-\nment • item")])]
        )
    ...
    def test_covid_hyphen_preserved(self) -> None:
        raw_doc = make_raw_doc(
            pages=[make_page(1, blocks=[make_block("COVID-19 patients")])]
        )
        result = clean_document(raw_doc)
