...
class _FakeDiscoveryClient:
    ...
    async def discover_source(self, source) -> list[DiscoveredDocument]:  # noqa: ANN001
...
def test_sync_downloads_missing_document_and_ingests(monkeypatch, tmp_path: Path) -> None:
    doc = _make_doc(
    ...
    async def fake_download(**kwargs):  # noqa: ANN003
...
def test_sync_skips_unchanged_document(monkeypatch, tmp_path: Path) -> None:
    ...
    async def fake_download(**kwargs):  # noqa: ANN003
        calls["download"] += 1
        return ("unused", {"etag": "etag-same", "last_modified": "Wed", "content_length": "100"})
