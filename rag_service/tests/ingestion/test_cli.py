...
class TestSyncWebCommand:
    @pytest.fixture
    def runner(self) -> CliRunner:
        return CliRunner()

    def test_requires_db_url_when_dry_run_without_env(self, runner: CliRunner) -> None:
        with patch.dict(os.environ, {}, clear=True):
            result = runner.invoke(cli, ["sync-web", "--dry-run"])
        assert result.exit_code == 1
        assert "--db-url" in result.output

    def test_passes_source_filter_to_sync_once(self, runner: CliRunner) -> None:
        fake_result = {
            "summary": {
                "discovered_count": 1,
                ...
                "ingest_succeeded_count": 1,
                "ingest_failed_count": 0,
                "errors": [],
            }
        }
        with (
            patch("src.ingestion.cli._resolve_db_url", return_value="postgresql://x"),
            patch("src.ingestion.cli.GuidelineWebSync") as sync_cls,
        ):
            captured: dict[str, Any] = {}

            async def fake_sync_once(**kwargs: Any) -> dict[str, Any]:
                captured.update(kwargs)
                return fake_result

            sync_cls.return_value.sync_once = fake_sync_once
            result = runner.invoke(
                cli,
                ["sync-web", "--source", "NICE", "--source", "BSR", "--dry-run"],
            )

        assert result.exit_code == 0
        assert captured["source_names"] == {"NICE", "BSR"}

    def test_sync_web_exits_on_errors_when_not_dry_run(self, runner: CliRunner) -> None:
        fake_result = {
            "summary": {
                "discovered_count": 1,
        ...
        with (
            ...
            sync_cls.return_value.sync_once = fake_sync_once
            result = runner.invoke(cli, ["sync-web"])

        assert result.exit_code == 1
        assert "Errors:" in result.output

    def test_sync_web_dry_run_does_not_exit_on_errors(self, runner: CliRunner) -> None:
        fake_result = {
            "summary": {
                "discovered_count": 1,
                "downloaded_new_count": 0,
        ...
        assert result.exit_code == 0
        assert "Web sync complete" in result.output
