...
_fake_config.CLOUD_LLM_MODEL = "fake-cloud-model"  # type: ignore[attr-defined]
_fake_config.FORCE_CLOUD_LLM = False  # type: ignore[attr-defined]
_fake_config.LLM_MAX_TOKENS = 512  # type: ignore[attr-defined]
_fake_config.LLM_ROUTE_THRESHOLD = 0.65  # type: ignore[attr-defined]
_fake_config.LOCAL_LLM_MODEL = "fake-local-model"  # type: ignore[attr-defined]
...
try:
    ...
    for _module_name, _original in _ORIGINAL_MODULES.items():
        ...
        if "." in _module_name:
            ...
            if _parent_module is not None and hasattr(_parent_module, _child_name):
                try:
                    delattr(_parent_module, _child_name)
