...
def test_grounded_prompt_with_file_only_uses_with_citations_hint() -> None:
...
def test_grounded_prompt_orders_sections_with_chunks_and_files() -> None:
    prompt = prompts.build_grounded_prompt(
        "Question?",
...
def test_revision_prompt_no_context_no_files_uses_no_citations_hint() -> None:
...
def test_revision_prompt_files_only_uses_with_citations_hint_and_order() -> None:
    prompt = prompts.build_revision_prompt(
        original_question="q",
        previous_answer="a",
        specialist_feedback="f",
...
def test_new_instructions_require_part_coverage_and_scope_controls() -> None:
    ...
    assert "ACTIONABLE NEXT STEP" in instructions
    assert "include one explicit 'Next step:' sentence" in instructions
    assert "For each recommended test or imaging item" in instructions
    assert "provide interpretation only when directly supported" in instructions
