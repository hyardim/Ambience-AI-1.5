...
describe('adminChatsFilters', () => {
  it('replaces only the matching chat', () => {
    ...
    expect(replaceAdminChat([...chats], 2, updated as never)).toEqual([chats[0], updated]);
  });
  ...
  it('filters by title or owner identifier', () => {
    ...
    expect(
      filterAdminChats([{ id: 3, title: '', owner_identifier: 'owner-3' }] as never, 'owner-3'),
    ).toHaveLength(1);
    expect(
      filterAdminChats([{ id: 4, title: '', owner_identifier: null }] as never, 'missing'),
    ).toHaveLength(0);
  });
});
