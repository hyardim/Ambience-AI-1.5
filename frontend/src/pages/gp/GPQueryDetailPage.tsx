...
export function GPQueryDetailPage() {
  ...
  const shouldPoll =
    (hasPendingAIResponse || hasRevisionInProgress || isInReview) && !streamConnected && !sending;
  ...
  const fetchChat = useCallback(async () => {
    ...
    try {
      ...
      setChat(found);
      const mapped = found.messages.map((m) =>
    ...
    try {
      ...
      if (files && files.length > 0) {
        ...
        if (failures.length > 0) {
          setError(`Message sent, but some files failed to upload: ${failures.join(' | ')}`);
        }
      }
        ...
        <div className="bg-white rounded-xl shadow-sm flex-1 flex flex-col overflow-hidden">
          ...
          <div className="p-6 border-b border-gray-200">
              ...
              <div className="mt-4 p-4 border border-gray-200 rounded-lg bg-gray-50">
                <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setEditMeta((prev) => ({ ...prev, specialty: e.target.value || null }))
                    }
                    className="px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-[var(--nhs-blue)] focus:border-transparent"
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      setEditMeta((prev) => ({ ...prev, severity: e.target.value || null }))
                    }
            ...
            {chat.status === 'submitted' && (
              <div className="mt-4 flex items-center gap-2 bg-amber-50 text-amber-800 px-4 py-3 rounded-lg border border-amber-200">
                ...
                <p className="text-sm">
                  This consultation has been submitted for specialist review. You will be notified
                  once a specialist responds.
                </p>
              </div>
            )}
            {(chat.status === 'assigned' || chat.status === 'reviewing') && (
              <div className="mt-4 flex items-center gap-2 bg-blue-50 text-blue-800 px-4 py-3 rounded-lg border border-blue-200">
                <ClipboardCheck className="w-5 h-5 shrink-0" />
                <p className="text-sm">
                  A specialist is currently reviewing this consultation. Individual AI response
                  statuses are shown on each message below.
                </p>
              </div>
            )}
            {chat.status === 'approved' && (
              <div className="mt-4 flex items-center gap-2 bg-green-50 text-green-800 px-4 py-3 rounded-lg border border-green-200">
                <ClipboardCheck className="w-5 h-5 shrink-0" />
                <p className="text-sm">
                  This consultation has been approved by a specialist. Approved AI responses are
                  marked below.
                </p>
              </div>
            )}
            {chat.status === 'rejected' && (
              <div className="mt-4 flex items-center gap-2 bg-red-50 text-red-800 px-4 py-3 rounded-lg border border-red-200">
                <ClipboardCheck className="w-5 h-5 shrink-0" />
                <p className="text-sm">
                  This consultation has been rejected by a specialist. See the messages below for
                  details.
                </p>
              </div>
            )}
          ...
          <div ref={messagesContainerRef} className="flex-1 overflow-y-auto p-6 space-y-6">
            {messages.map((message, index) => {
              ...
              const timestampKey =
                message.timestamp instanceof Date ? message.timestamp.getTime() : '';
              return (
                <ChatMessage
          ...
          {(chat.status === 'open' || chat.status === 'submitted') && (
            <div className="border-t border-gray-200">
                ...
                <div className="px-4 pt-3 pb-0">
                  ...
                  <div className="flex flex-wrap gap-1.5">
                    {chat.files.map((f) => (
                      <span
                        key={f.id}
                        className="inline-flex items-center gap-1 px-2 py-0.5 bg-blue-50 border border-blue-100 rounded text-xs text-blue-700"
                      >
                        {f.filename}
                        {f.file_size ? (
              ...
              <p className="px-4 pt-3 text-xs text-gray-500">
                Max 3MB per file · PDF, TXT, MD, RTF, DOC, DOCX, CSV, JSON, XML
              </p>
              <ChatInput
