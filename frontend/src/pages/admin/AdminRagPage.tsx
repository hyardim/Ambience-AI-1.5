...
export default function AdminRagPage() {
  ...
  const [sortKey, setSortKey] = useState<'source_name' | 'chunk_count' | 'latest_ingestion'>(
    'latest_ingestion',
  );
  ...
  const sourceOptions = Array.from(
    new Set(data?.documents.map((doc) => doc.source_name).filter(Boolean) ?? []),
  ).sort();
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <div>
                      ...
                      <select
                        ...
                        onChange={(e) =>
                          setSortKey(
                            e.target.value as 'source_name' | 'chunk_count' | 'latest_ingestion',
                          )
                        }
                        className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-[var(--nhs-blue)] focus:border-transparent text-sm"
                      >
                        ...
                        {visibleDocuments.map((doc) => (
                          <tr key={doc.doc_id} className="text-gray-700">
                            ...
                            <td className="py-2.5 text-gray-500 text-xs">
                              {formatTimestamp(doc.latest_ingestion)}
                            </td>
                          </tr>
                        ))}
                      </tbody>
